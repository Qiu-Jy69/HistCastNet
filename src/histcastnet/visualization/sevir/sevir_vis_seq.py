...
def visualize_result(
    ...
    if vis_hits_misses_fas:
        fig, ax = plt.subplots(
            nrows=2 + 3 * len(pred_seq_list), ncols=ncols, figsize=figsize
        )
    else:
        fig, ax = plt.subplots(
            nrows=2 + len(pred_seq_list), ncols=ncols, figsize=figsize
        )
    ...
    for i in range(0, max_len, plot_stride):
        if i < in_len:
            ...
            ax[0][i // plot_stride].imshow(
                xt, **cmap_dict("vil"), interpolation="nearest"
            )
        else:
    ...
    y_preds = [
        pred_seq[idx : idx + 1] * norm["scale"] + norm["shift"]
    ...
    if vis_hits_misses_fas:
        for k in range(len(pred_seq_list)):
            for i in range(0, max_len, plot_stride):
                if i < out_len:
                    ax[2 + 3 * k][i // plot_stride].imshow(
                        y_preds[k][0, :, :, i],
                        **cmap_dict("vil"),
                        interpolation="nearest",
                    )
                    plot_hit_miss_fa(
                        ax[2 + 1 + 3 * k][i // plot_stride],
                        target_seq[0, :, :, i],
                        y_preds[k][0, :, :, i],
                        vis_thresh,
                    )
                    plot_hit_miss_fa_all_thresholds(
                        ax[2 + 2 + 3 * k][i // plot_stride],
                        target_seq[0, :, :, i],
                        y_preds[k][0, :, :, i],
                    )
                else:
            ...
            ax[2 + 1 + 3 * k][0].set_ylabel(
                label_list[k] + f"\nScores\nThresh={vis_thresh}", fontsize=fs
            )
            ax[2 + 2 + 3 * k][0].set_ylabel(
    ...
    for i in range(0, max_len, plot_stride):
        if i < out_len:
            ax[-1][i // plot_stride].set_title(
                f"{int(interval_real_time * (i + plot_stride))} Minutes", y=-0.25
            )
...
def save_example_vis_results(
    ...
    if save_individual_frames:
        ...
        frame_dir = os.path.join(save_dir, f"{save_prefix}_frames")
        os.makedirs(frame_dir, exist_ok=True)

        in_item = in_seq[idx] * norm["scale"] + norm["shift"]
        target_item = target_seq[idx] * norm["scale"] + norm["shift"]
        pred_item = pred_seq[idx] * norm["scale"] + norm["shift"]

        for t in range(in_item.shape[-1]):

            rgba_in = vil_cmap(vil_norm(in_item[:, :, t]))
            plt.imsave(os.path.join(frame_dir, f"in_t{t:03d}.png"), rgba_in)

        for t in range(target_item.shape[-1]):
            rgba_target = vil_cmap(vil_norm(target_item[:, :, t]))
            plt.imsave(os.path.join(frame_dir, f"target_t{t:03d}.png"), rgba_target)

        for t in range(pred_item.shape[-1]):
            rgba_pred = vil_cmap(vil_norm(pred_item[:, :, t]))
            plt.imsave(os.path.join(frame_dir, f"pred_t{t:03d}.png"), rgba_pred)
