...
class SEVIRDataLoader:
    ...
    def __init__(
        ...
        self.catalog_raw = self.catalog.copy()

        if (
            hasattr(self.catalog["time_utc"], "dt")
            and getattr(self.catalog["time_utc"].dt, "tz", None) is not None
        ):
            self.catalog["time_utc"] = self.catalog["time_utc"].dt.tz_localize(None)
    ...
    def _open_files(self, verbose: bool = True):
        df = getattr(self, "catalog_raw", None)
        if df is None:
            df = self.catalog

        if df is None or len(df) == 0:
            raise RuntimeError(
                "catalog_raw is empty; file_name and img_type cannot be read."
            )

        if "file_name" not in df.columns:
            raise RuntimeError(f"catalog has no file_name column: {list(df.columns)}")

        has_img_type = "img_type" in df.columns
        imgt = getattr(self, "data_types", None)

        self._hdf_files = {}

        if has_img_type and imgt is not None:
            type_list = list(imgt)
        else:
            type_list = [None]
        ...
        for t in type_list:
            if t is None:
                ...
                df_t = df[df["img_type"].str.lower() == t.lower()]
                if len(df_t) == 0:
                    if verbose:
                        print(f"[WARN] No samples found for img_type={t}.")
                    continue

            files_t = df_t["file_name"].dropna().unique()

            for f in files_t:
                file_path = os.path.join(self.sevir_data_dir, f)
                if not os.path.exists(file_path):
                    continue

                if f in self._hdf_files:
                    continue
        ...
        if len(self._hdf_files) == 0:
            raise RuntimeError(
                f"No HDF5 files could be opened from {self.sevir_data_dir}."
            )
        ...
        for key, data in data_dict.items():
            if key in data_types:
                if isinstance(data, torch.Tensor):
                    ...
                    raise ValueError(
                        f"Invalid data type: {type(data)}. Should be torch.Tensor or np.ndarray"
                    )
            else:  # key == "mask"
                ret_dict[key] = data
        return ret_dict
        ...
        for sampled_idx in sampled_idx_list:
            ...
            for imgt_idx, imgt in enumerate(self.data_types):
                ...
                if imgt in ret_dict:
                    ...
                    t1 = ret_dict[imgt].shape[-1]
                    t2 = sampled_seq.shape[-1]
                    t_min = min(t1, t2)

                    ret_dict[imgt] = np.concatenate(
                        (ret_dict[imgt][..., :t_min], sampled_seq[..., :t_min]), axis=0
                    )
                else:
                    ret_dict.update({imgt: sampled_seq})
