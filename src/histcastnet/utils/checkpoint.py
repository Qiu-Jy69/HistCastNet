...
def average_checkpoints(checkpoint_paths: List[str] = None):
    ...
    if len(checkpoint_paths) > 1:
        ...
        for k in avg_state_dict:
            avg_state_dict[k].clamp_(float32_info.min, float32_info.max).to(
                dtype=torch.float32
            )
    else:
...
def average_pl_checkpoints(
    pl_checkpoint_paths: List[str] = None, delete_prefix_len: int = len("")
):
    r"""Code is adapted from https://github.com/awslabs/autogluon/blob/a818545e047f2bcda13569568a7fd611abdfc876/multimodal/src/autogluon/multimodal/utils/checkpoint.py#L13
    ...
    if len(pl_checkpoint_paths) > 1:
        ...
        avg_state_dict = pl_ckpt_to_pytorch_state_dict(
            pl_checkpoint_paths[0],
            map_location=torch.device("cpu"),
            delete_prefix_len=delete_prefix_len,
        )
...
def pl_ckpt_to_pytorch_state_dict(
    ...
    pytorch_state_dict = {
        key[delete_prefix_len:]: val for key, val in pl_ckpt_state_dict.items()
    }
