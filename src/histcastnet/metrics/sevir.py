...
class SEVIRSkillScore(Metric):
    ...
    def __init__(
        ...
        self.add_state("cns", default=torch.zeros(state_shape), dist_reduce_fx="sum")
    ...
    @staticmethod
    def ets(hits, misses, fas, cns, eps):
        r"""Equitable Threat Score from the 2x2 contingency table (same reduce as H/M/FA)."""
        h = hits.float()
        m = misses.float()
        f = fas.float()
        c = cns.float()
        ...
        h_ref = (h + f) * (h + m) / n
        denom = h + m + f - h_ref + eps
        return (h - h_ref) / denom
    ...
    def calc_seq_hits_misses_fas(self, pred, target, threshold):
        ...
        Returns
        -------
        hits, misses, fas, cns:  torch.Tensor
            each has shape (seq_len, ) if keep_seq_len_dim else scalar per-threshold slice
        """
        with torch.no_grad():
            ...
            cns = torch.sum(
                (1 - t) * (1 - p), dim=self.hits_misses_fas_reduce_dims
            ).int()
    ...
    def preprocess(self, pred, target):
        if self.preprocess_type == "sevir":
            pred = SEVIRDataLoader.process_data_dict_back(
                data_dict={"vil": pred.detach().float()}
            )["vil"]
            target = SEVIRDataLoader.process_data_dict_back(
                data_dict={"vil": target.detach().float()}
            )["vil"]
        else:
    ...
    def update(self, pred: torch.Tensor, target: torch.Tensor):
        ...
        for i, threshold in enumerate(self.threshold_list):
            hits, misses, fas, cns = self.calc_seq_hits_misses_fas(
    ...
    def compute(self):
        ...
        for metrics in self.metrics_list:
            ...
            if metrics == "ets":
                scores = self.ets(self.hits, self.misses, self.fas, self.cns, self.eps)
            elif metrics in metrics_dict:
                scores = metrics_dict[metrics](
                    self.hits, self.misses, self.fas, self.eps
                )
            else:
                raise ValueError(f"Unknown metric in metrics_list: {metrics}")
